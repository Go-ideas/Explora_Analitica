...
def _stable_factor_id(
    row: pd.Series,
    question_col: str | None,
    type_col: str | None,
    variables_col: str | None,
    index: int,
) -> str:
    raw = "_".join(
        [
            _clean(row.get(question_col)) if question_col else "",
            _clean(row.get(type_col)) if type_col else "",
            _clean(row.get(variables_col)) if variables_col else "",
        ]
    )
    value = normalize_text(raw).strip("_")
    return value or f"factor_{index}"
