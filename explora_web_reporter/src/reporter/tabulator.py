...
def available_questions(
    ...
    questions = _merge_grid_questions(db_path, questions)
...
def _merge_grid_questions(
    db_path: Path,
    questions: pd.DataFrame,
) -> pd.DataFrame:
    grids = _safe_read_table(db_path, "grids")
    if grids.empty or "grid_id" not in grids:
        return questions
    result = questions.copy()
    if "pregunta_id" not in result:
        result["pregunta_id"] = ""
    existing = set(result["pregunta_id"].dropna().astype(str))
    additions = []
    for _, grid in grids.iterrows():
        grid_id = str(grid.get("grid_id") or "").strip()
        if not grid_id:
            continue
        grid_type = _grid_question_type(grid)
        row = {
            "pregunta_id": grid_id,
            "numero_pregunta": grid.get("pregunta_padre") or grid_id,
            "texto_pregunta": grid.get("texto_pregunta") or grid_id,
            "tipo_pregunta": grid_type,
            "tipo_calculo": (
                "Multirrespuesta agrupada"
                if grid_type == "GRID_RM/LOOP_RM"
                else grid.get("metrica_grid_recomendada")
                or "Distribución grid"
            ),
            "usar_en_dashboard": 1,
            "grid_id": grid_id,
            "tipo_estructura_grid": grid.get("tipo_estructura_grid") or grid_type,
            "es_grid_rm_loop": grid.get("es_grid_rm_loop", 0),
            "orden_cuestionario": None,
        }
        if grid_id in existing:
            mask = result["pregunta_id"].astype(str).eq(grid_id)
...
def _grid_question_type(row: pd.Series) -> str:
    structure = normalize_grid_text(
        " ".join(
    ...
    if bool(pd.to_numeric(pd.Series([row.get("es_grid_rm_loop")]), errors="coerce").fillna(0).iloc[0]):
        return "GRID_RM/LOOP_RM"
    if "grid_rm" in structure or "loop_rm" in structure:
        return "GRID_RM/LOOP_RM"
    if "rm" in structure and ("loop" in structure or "opcion" in structure or "opción" in structure):
        return "GRID_RM/LOOP_RM"
    if "grid_escala" in structure:
        return "GRID_ESCALA"
    if "loop_ru" in structure:
        return "LOOP_RU"
    if "loop_numerico" in structure or "loop_numérico" in structure:
        return "LOOP_NUMERICO"
    if "loop_rango" in structure:
        return "LOOP_RANGO"
    return "GRID_ESCALA"


def normalize_grid_text(value: object) -> str:
...
def _infer_question_type(
    ...
    normalized_metadata = normalize_grid_text(metadata)
    if any(token in normalized_metadata for token in ("grid_rm", "loop_rm")):
        return "GRID_RM/LOOP_RM"
    if "grid_escala" in normalized_metadata:
        return "GRID_ESCALA"
    if "loop_ru" in normalized_metadata:
        return "LOOP_RU"
    if "loop_numerico" in normalized_metadata or "loop_numérico" in normalized_metadata:
        return "LOOP_NUMERICO"
    if "loop_rango" in normalized_metadata:
        return "LOOP_RANGO"
    if "nps" in metadata:
        return "NPS"
    if question_id in scale_ids or any(
        token in metadata
        for token in ("escala", "grid", "media", "top2box", "bottombox")
    ):
        return "Escala"
    if (
...
def generate_report(
    ...
    *,
    ...
    grid_loop_raw = _read_question_table(
        db_path, "respuestas_grid_loop", question_id
    )
    ...
    if question_type_display == "GRID_RM/LOOP_RM":
        if not grid_loop_raw.empty:
            grid_loop_raw = _grid_loop_with_response_labels(
                grid_loop_raw, question_id
            )
        else:
            rm_raw = _apply_grid_rm_metadata(db_path, question_id, rm_raw)
    section_display = _derive_section(
        question_row.get("seccion"), question_id
    )
    metadata_text = " ".join(
        str(question_row.get(column) or "")
        for column in ("tipo_pregunta", "tipo_calculo")
    )
    metadata_is_scale = question_type_display in {"Escala", "NPS", "GRID_ESCALA"} or any(
        token in metadata_text.lower()
        for token in ("escala", "grid_escala", "media", "top2box", "bottombox", "nps")
    )
    ...
    if question_type_display == "RM":
        question_type += " __rm__"
    if question_type_display == "GRID_RM/LOOP_RM":
        question_type += " grid_rm loop_rm multirrespuesta agrupada __rm__"
    if question_type_display in {"Escala", "NPS", "GRID_ESCALA"}:
    ...
    if question_type_display == "GRID_RM/LOOP_RM" and any(
        item.startswith("Ranking ") for item in valid_calculations
    ):
        if "% respondentes" not in valid_calculations:
            valid_calculations.insert(0, "% respondentes")
    ...
    if not grid_loop_raw.empty and {
        "id_respondente",
        "base_valida",
    }.issubset(grid_loop_raw.columns):
        valid_grid = pd.to_numeric(
            grid_loop_raw["base_valida"], errors="coerce"
        ).fillna(0).astype(bool)
        source_ids.update(
            grid_loop_raw.loc[valid_grid, "id_respondente"].astype(str)
        )
    ...
    summaries = {
        ...
        "rm": (
            _grid_loop_rm_summary(grid_loop_data)
            if question_type_display == "GRID_RM/LOOP_RM"
            and not grid_loop_data.empty
            else rm_summary(rm_data)
    ...
    if question_type_display == "GRID_RM/LOOP_RM":
        table = _format_grid_rm_table(table, valid_calculations)
    ...
    if question_type_display == "GRID_RM/LOOP_RM":
        notes.append(
            "Esta pregunta es GRID_RM/LOOP_RM: matriz multirrespuesta "
            "por entidad y opción. Se tabula como % de respondentes/"
            "menciones, no como escala."
        )
        if significance_display != "none":
            warnings.append(
                "La significancia para GRID_RM/LOOP_RM se aplica "
                "solo sobre proporciones de selección. No se calculan medias."
            )
    return ReportResult(
...
def _apply_grid_rm_metadata(
    db_path: Path,
    question_id: str,
    rm_raw: pd.DataFrame,
) -> pd.DataFrame:
    if rm_raw.empty or "variable_origen" not in rm_raw:
        return rm_raw
    try:
        items = read_sql(
            db_path,
            'SELECT * FROM "grid_items" WHERE grid_id = ?',
            (str(question_id),),
        )
    except Exception:
        return rm_raw
    if items.empty or "variable" not in items:
        return rm_raw
    keep = [
        column
        for column in (
    ...
    metadata = items[keep].drop_duplicates("variable")
    result = rm_raw.merge(
        metadata,
        left_on="variable_origen",
        right_on="variable",
        how="left",
    )
    entity = result.get("entidad_loop", pd.Series("", index=result.index))
    option = result.get("label_opcion_rm", pd.Series("", index=result.index))
    fallback = result.get("texto_fila_grid", pd.Series("", index=result.index))
    raw_label = result.get("respuesta_label", pd.Series("", index=result.index))
    result["_grid_entidad"] = entity.fillna("").astype(str).str.strip()
    result["_grid_opcion"] = option.fillna("").astype(str).str.strip()
    empty_option = result["_grid_opcion"].eq("")
    result.loc[empty_option, "_grid_opcion"] = (
        fallback.fillna("").astype(str).str.strip().where(
            fallback.fillna("").astype(str).str.strip().ne(""),
            raw_label.fillna("").astype(str).str.strip(),
        )
    )
    result["_grid_entidad"] = result["_grid_entidad"].where(
        result["_grid_entidad"].ne(""), str(question_id)
    )
    result["respuesta_label"] = (
        result["_grid_entidad"] + " | " + result["_grid_opcion"]
    ).str.strip(" |")
    return result


def _grid_loop_with_response_labels(
    grid_loop_raw: pd.DataFrame,
    question_id: str,
) -> pd.DataFrame:
    result = grid_loop_raw.copy()
    entity = result.get("entidad_loop", pd.Series("", index=result.index))
    option = result.get("label_opcion", pd.Series("", index=result.index))
    code = result.get("codigo_opcion", pd.Series("", index=result.index))
    result["_grid_entidad"] = entity.fillna("").astype(str).str.strip()
    result["_grid_opcion"] = option.fillna("").astype(str).str.strip()
    empty_option = result["_grid_opcion"].eq("")
    result.loc[empty_option, "_grid_opcion"] = (
        code.fillna("").astype(str).str.strip()
    )
    result["_grid_entidad"] = result["_grid_entidad"].where(
        result["_grid_entidad"].ne(""), str(question_id)
    )
...
def _grid_loop_rm_summary(data: pd.DataFrame) -> pd.DataFrame:
    if data.empty or "respuesta" not in data:
        return pd.DataFrame()
    work = data.copy()
    work["_selected"] = pd.to_numeric(
        work.get("seleccionado", 0), errors="coerce"
    ).fillna(0).astype(bool)
    work["_base_valid"] = pd.to_numeric(
        work.get("base_valida", 0), errors="coerce"
    ).fillna(0).astype(bool)
    bases = (
        work.loc[work["_base_valid"]]
        .drop_duplicates(["banner", "id_respondente"])
        .groupby("banner")["id_respondente"]
        .nunique()
    )
    weighted_bases = (
        work.loc[work["_base_valid"]]
        .drop_duplicates(["banner", "id_respondente"])
        .groupby("banner")["_peso"]
        .sum()
    )
    rows = []
    for (banner, response), group in work.groupby(
        ["banner", "respuesta"], dropna=False, sort=False
    ):
        ...
        rows.append(
            {
                "banner": banner,
                "respuesta": response,
                "menciones": int(len(selected)),
                "respondentes": int(
                    selected["id_respondente"].nunique()
                ),
                "menciones_ponderadas": float(
                    selected["_peso"].sum()
                ),
            }
        )
    ...
    if result.empty:
        return result
    result["base"] = result["banner"].map(bases).fillna(0).astype(int)
    result["base_ponderada"] = (
        result["banner"].map(weighted_bases).fillna(0.0)
    )
    result["pct_respondentes"] = (
        result["respondentes"] / result["base"].replace(0, pd.NA)
    ).fillna(0)
    result["base_menciones"] = result.groupby("banner")[
        "menciones"
    ].transform("sum")
    result["pct_menciones"] = (
        result["menciones"]
        / result["base_menciones"].replace(0, pd.NA)
    ).fillna(0)
    result["pct_respondentes_ponderado"] = (
        result["menciones_ponderadas"]
        / result["base_ponderada"].replace(0, pd.NA)
    ).fillna(0)
    weighted_mentions = result.groupby("banner")[
        "menciones_ponderadas"
    ].transform("sum")
    result["pct_menciones_ponderado"] = (
        result["menciones_ponderadas"]
        / weighted_mentions.replace(0, pd.NA)
    ).fillna(0)
    return result


def _format_grid_rm_table(
    table: pd.DataFrame,
    calculations: list[str],
) -> pd.DataFrame:
    if table.empty or "Respuesta" not in table:
        return table
    result = table.copy()
    split = result["Respuesta"].astype(str).str.split(" | ", n=1, regex=False)
    result.insert(0, "Entidad", split.str[0])
    result.insert(
        1,
        "Opción",
        split.str[1].fillna(result["Respuesta"].astype(str)),
    )
    result = result.drop(columns=["Respuesta"])
    rank_metric = None
    if "Ranking opciones por entidad" in calculations:
        rank_metric = _first_metric_column(result, "% respondentes")
    elif "Ranking entidades por opción" in calculations:
        rank_metric = _first_metric_column(result, "% respondentes")
    if rank_metric:
        if "Ranking opciones por entidad" in calculations:
            result["Rank"] = result.groupby("Entidad")[rank_metric].rank(
                ascending=False,
                method="dense",
            )
        else:
            result["Rank"] = result.groupby("Opción")[rank_metric].rank(
                ascending=False,
                method="dense",
            )
    return result
...
def _first_metric_column(
    ...
    for column in table.columns:
        if str(column).endswith(f" | {metric}"):
            return str(column)
    return None
...
def _multi_significance(
    ...
    for calculation in (
        ...
        if calculation not in calculations or rm.empty:
            continue
        mentions = "menciones" in calculation.lower()
        sig_summary = rm.copy()
...
def _method_notes(
    ...
    if any(
        item.startswith("RM ")
        or item in {"% respondentes", "Menciones", "% menciones"}
        for item in calculations
    ):
        notes.append(
    ...
    if "RM % Menciones" in calculations or "% menciones" in calculations:
        notes.append(
