...
def config_entries(
    ...
    if kind in {"banner", "filtro"}:
        ...
        return _safe_banner_filter_entries(db_path, entries)
    try:
...
def _safe_banner_filter_entries(
    db_path: Path,
    entries: pd.DataFrame,
) -> pd.DataFrame:
    if entries.empty or "variable" not in entries:
        return entries
    try:
    ...
    for _, row in entries.iterrows():
        ...
        reason = _metadata_exclusion_reason(metadata.get(variable, {}))
        if not reason:
            reason = banner_filter_exclusion_reason(
                respondentes, variable
            )
        keep.append(not reason)
    return entries.loc[keep].reset_index(drop=True)


def _metadata_exclusion_reason(metadata: dict) -> str:
    if not metadata:
        return ""
    if truthy(metadata.get("es_item_grid")):
        return "Ítem individual de grid/loop"
    if truthy(metadata.get("es_grid_rm_loop")):
        return "Opción individual de GRID_RM/LOOP_RM"
    if truthy(metadata.get("es_abierta_asociada")):
        return "Abierta asociada"
    classification = normalize_text(
        metadata.get("clasificacion_analitica")
    )
    if classification in {
        ...
        return "Variable no segura para banner/filtro"
    if not truthy(metadata.get("usar_en_dashboard")):
        return "No marcada para dashboard"
    return ""
