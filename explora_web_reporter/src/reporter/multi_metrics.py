...
def validate_calculations_for_question(
    ...
    is_rm = any(token in text for token in ("__rm__", "multirrespuesta", "multiple", " rm"))
    is_grid_rm = any(
        token in text
        for token in ("grid_rm", "loop_rm", "multirrespuesta agrupada")
    )
    is_scale = any(
        token in text
        for token in (
    ...
    for raw_calculation in selected_calculations:
        ...
        applies = True
        if is_grid_rm:
            applies = calculation in grid_rm_allowed
        elif calculation.startswith("RM "):
            applies = is_rm
        elif calculation in {
            "Media",
            "Desviación estándar",
...
def merge_significance_into_table(
    ...
    response_column = result.columns[0]
    categories = []
...
def _response_metric_specs(
    ...
    for calculation in calculations:
        ...
        elif calculation == "% respondentes":
            column = "pct_respondentes"
        elif calculation == "Menciones":
            column = "menciones"
        elif calculation == "% menciones":
            column = "pct_menciones"
        elif calculation == "Multirrespuesta agrupada":
            column = "pct_respondentes"
        else:
            continue
