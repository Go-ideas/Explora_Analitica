...
from src.web_canonical.adapter import CanonicalWebAdapterError
from src.web_canonical.comparison import DualRunComparisonResult
...
from src.web_canonical import project_canonical_result
...
def render() -> None:
    ...
    if update:
        ...
        try:
            ...
            _store_analysis_result(result, execution_mode, current_binding)
            if execution_mode == ExecutionMode.LEGACY.value:
                previous = st.session_state.get("failed_analysis_binding")
                if isinstance(previous, AnalysisSessionBinding):
                    record_rollback_event(
                        st.session_state,
                        from_binding=previous,
                        to_binding=current_binding,
                    )
        except Exception as exc:
            mark_analysis_failure(st.session_state, current_binding, exc)
            st.error(f"No fue posible generar el reporte: {exc}")

    if not _has_current_analysis(current_binding):
        st.info(
            "La tabla aparecerá aquí después de seleccionar la "
            "configuración."
        )
    else:
        _render_current_analysis(
            key_prefix="report",
...
def _canonical_result_for_settings(
    question_id: str,
) -> CanonicalResult | None:
    direct = st.session_state.get("canonical_result")
    if isinstance(direct, CanonicalResult):
        return direct
    registry = st.session_state.get("canonical_results")
    if isinstance(registry, dict):
        candidate = registry.get(question_id)
        if isinstance(candidate, CanonicalResult):
            return candidate
    return None
...
def _analysis_binding(
    ...
    return analysis_session_binding(
        execution_mode=execution_mode,
        request_identity=str(observability["request_identity"]),
        request=request,
...
def _store_analysis_result(
    result: object,
    execution_mode: str,
    request_binding: AnalysisSessionBinding,
) -> None:
    mode = ExecutionMode(execution_mode)
    if mode in {ExecutionMode.LEGACY, ExecutionMode.CORE_WRAPPER}:
        st.session_state.current_report = result
        st.session_state.current_report_legacy = result
        st.session_state.current_canonical_result = None
        st.session_state.current_canonical_projection = None
        st.session_state.current_dual_run = None
        st.session_state.current_result_contract = "reportresult_legacy"
        store_current_analysis_binding(st.session_state, request_binding)
        return
    if mode is ExecutionMode.CANONICAL_V1:
        canonical = _require_canonical_result(result)
        projection = project_canonical_result(canonical)
        enriched_binding = replace(
            request_binding,
            result_run_id=canonical.result_run_id,
            result_fingerprint=canonical.result_fingerprint,
        )
        st.session_state.current_report = None
        st.session_state.current_report_legacy = None
        st.session_state.current_canonical_result = canonical
        st.session_state.current_canonical_projection = projection
        st.session_state.current_dual_run = None
        st.session_state.current_result_contract = "canonical_v1"
        store_current_analysis_binding(st.session_state, enriched_binding)
        return
    if mode is ExecutionMode.DUAL_RUN:
        if not isinstance(result, DualRunExecution):
            raise CanonicalResultRequiredError(
                "DUAL_RUN requires separate Legacy and Canonical results."
            )
        projection = project_canonical_result(result.canonical_result)
        enriched_binding = replace(
            request_binding,
            result_run_id=result.canonical_result.result_run_id,
            result_fingerprint=result.canonical_result.result_fingerprint,
        )
        st.session_state.current_report = None
        st.session_state.current_report_legacy = result.legacy_result
        st.session_state.current_canonical_result = (
            result.canonical_result
        )
        st.session_state.current_canonical_projection = projection
        st.session_state.current_dual_run = result
        st.session_state.current_result_contract = "dual_run"
        store_current_analysis_binding(st.session_state, enriched_binding)
        return
    raise AssertionError(f"Unhandled execution mode: {execution_mode}")


def _has_current_analysis(current_binding: AnalysisSessionBinding) -> bool:
    if not current_analysis_matches(st.session_state, current_binding):
        return False
    return bool(
        st.session_state.get("current_report") is not None
        or st.session_state.get("current_canonical_projection") is not None
    )


def _render_current_analysis(
    key_prefix: str,
    current_binding: AnalysisSessionBinding,
) -> None:
    if not current_analysis_matches(st.session_state, current_binding):
        st.info("La configuración actual requiere una nueva ejecución.")
        return
    contract = st.session_state.get(
        "current_result_contract", "reportresult_legacy"
    )
    if contract == "reportresult_legacy":
        report = st.session_state.get("current_report")
        if report is not None:
            render_report(report, key_prefix=key_prefix)
        return
    projection = st.session_state.get("current_canonical_projection")
    if isinstance(projection, CanonicalWebProjection):
        if contract == "dual_run":
            st.info(
                "DUAL_RUN ejecutó Legacy y Canonical por separado. "
                "La vista Canonical no se completa con celdas Legacy."
            )
        comparison = None
        dual = st.session_state.get("current_dual_run")
        if isinstance(dual, DualRunExecution):
...
def _require_canonical_result(result: object) -> CanonicalResult:
    if not isinstance(result, CanonicalResult):
        raise CanonicalResultRequiredError(
            "CANONICAL_V1 requires a CanonicalResult. "
            "Legacy fallback is prohibited."
        )
    return result


def render_canonical_projection(
    projection: CanonicalWebProjection,
    key_prefix: str,
    comparison: DualRunComparisonResult | None = None,
) -> None:
    st.divider()
    with st.container(border=True):
        st.caption(
            f"{projection.authority} · {projection.result_schema_version}"
        )
        st.markdown("### Resultado canónico")
        columns = st.columns(4)
        columns[0].metric("Proyecto", projection.project_id)
        columns[1].metric(
            "Run",
            projection.result_run_id[:16],
        )
        columns[2].metric(
            "Release",
            str(projection.release.get("qa_release_status", "UNKNOWN")),
        )
        columns[3].metric(
            "Releasable",
            "Sí" if projection.release.get("releasable") else "No",
        )
        st.caption(f"Cache key: {projection.cache_key}")
    if not projection.release.get("releasable"):
        st.warning(
            "Resultado canónico no liberable para salida oficial; "
            "vista disponible sólo para QA interna."
        )
    for warning in projection.warnings:
        st.warning(warning)
    if comparison is not None and comparison.blocks_migration:
        st.error("DUAL_RUN contiene POTENTIAL_REGRESSION.")
    if projection.table.empty:
        st.warning("CanonicalResult no contiene valores renderizables.")
        return
    st.markdown("### Tabla")
    render_report_table(projection.table)
    if not projection.bases.empty:
        with st.expander("Bases canónicas", expanded=False):
            st.dataframe(projection.bases, hide_index=True, width="stretch")
    if not projection.significance.empty:
        render_report_table(
            projection.significance,
            title="Significancia canónica",
        )
    if projection.qa:
        with st.expander("QA canónico", expanded=False):
            st.dataframe(
                pd.DataFrame([item.__dict__ for item in projection.qa]),
                hide_index=True,
                width="stretch",
            )
    st.download_button(
        (
            "Exportar resultado oficial canónico CSV"
            if projection.release.get("releasable")
            else "Exportar QA interna canónica CSV"
        ),
        data=(
            official_released_export_bytes(projection)
            if projection.release.get("releasable")
            else internal_qa_export_bytes(projection)
        ),
        file_name=safe_filename(
            projection.result_run_id,
            (
        ...
        mime="text/csv",
        key=f"{key_prefix}_canonical_download_{projection.result_run_id}",
    )
...
def render_report(report: ReportResult, key_prefix: str) -> None:
    ...
    st.download_button(
        ...
        file_name=safe_filename(report.question_id, "Reporte_Explora.xlsx"),
...
def _default_calculations(
    ...
    if "grid_rm" in normalized_type or "loop_rm" in normalized_type:
        return ["% respondentes", "Menciones"]
    if "grid_escala" in normalized_type:
        return ["Distribución grid", "Media", "Top2Box", "BottomBox"]
    if question_type == "RM":
        return [
            "n",
            "RM % Respondentes",
