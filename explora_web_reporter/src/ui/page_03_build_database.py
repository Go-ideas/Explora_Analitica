...
from src.structure.grid_loop_detector import validate_grid_loop_children
...
def render() -> None:
    ...
    validation = _validate_datamap_for_build(datamap, df_spss)
...
def _configuration_warning(
    ...
    if truthy(row.get("es_item_grid")) or truthy(
        row.get("es_grid_rm_loop")
    ):
        return "Ítem/opción de grid: usar desde pregunta padre"
    if truthy(row.get("es_abierta_asociada")):
        return "Abierta asociada a grid/loop"
    if unique == 0:
...
def _validate_datamap_for_build(
    datamap: pd.DataFrame,
    df_spss: pd.DataFrame,
) -> dict[str, bool]:
    required = ["variable", "clasificacion_analitica", "usar_en_dashboard"]
    missing = [col for col in required if col not in datamap.columns]
    if missing:
        st.error(f"El Datamap no contiene columnas requeridas: {missing}")
        return {"ok": False}
    ...
    sp_cols = set(df_spss.columns)
    not_in_spss = sorted(
        {
            variable
            for variable in variables
            if variable and variable not in sp_cols
        }
    )
    if not_in_spss:
        work = datamap.copy()
        work["_variable_clean"] = variables
        missing_rows = work[work["_variable_clean"].isin(not_in_spss)]
        allowed = {
            "Derivada",
            "Factor",
            "Score",
        ...
        blocking_rows = missing_rows[
            ~missing_rows["clasificacion_analitica"].astype(str).isin(allowed)
            & ~missing_rows.get("es_grid", pd.Series(False, index=missing_rows.index)).apply(truthy)
        ]
        if blocking_rows.empty:
            st.warning(
                "Hay variables del Datamap que no existen en SPSS, "
                "pero están clasificadas como derivadas, factores, "
                "padres de grid o excluidas."
            )
        else:
            preview = blocking_rows["_variable_clean"].head(20).tolist()
            st.error(
                "Variables del Datamap no encontradas en SPSS: "
                + ", ".join(preview)
            )
            return {"ok": False}
    grid_validation = validate_grid_loop_children(
        datamap_df=datamap,
        df_spss=df_spss,
        meta_spss=st.session_state.get("meta_spss"),
    )
    if grid_validation is not None and not grid_validation.empty:
        critical = grid_validation[
            grid_validation["bloquea"].apply(truthy)
        ]
        if not critical.empty:
            st.error(
                "Hay estructuras GRID/LOOP críticas que impiden "
            ...
            return {"ok": False}
        warnings = grid_validation[
            ~grid_validation["bloquea"].apply(truthy)
        ]
        if not warnings.empty:
            st.warning(
                "Hay advertencias metodológicas GRID/LOOP. "
                "Puedes construir, pero conviene revisarlas."
            )
            st.dataframe(
                warnings[
                    [
